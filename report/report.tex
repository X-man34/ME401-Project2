...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\textwidth}{\centering
    \textbf{System Flow} \\[0.5em]
    Wastewater Sludge $\rightarrow$ \fbox{Anaerobic Digester (CSTR)} $\xrightarrow{\text{Biogas (CH}_4\text{+CO}_2\text{)}}$ \fbox{CHP Rankine Plant} $\rightarrow$ Electricity + Heat \\[0.3em]
    \hspace{8em} $\uparrow$ \hspace{14em} $\downarrow$ \\
    \hspace{8em} $\longleftarrow$ \hspace{2em} Process Heat Feedback \hspace{2em} $\longleftarrow$
  }}
...
The simulation is implemented in Python 3.12 using the following libraries:
\begin{itemize}[noitemsep]
  \item \textbf{CoolProp 7.2}: Thermodynamic property evaluation for water/steam states
  \item \textbf{NumPy/SciPy}: Numerical computation, optimization (bounded scalar, differential evolution)
  \item \textbf{Matplotlib}: Figure generation
\end{itemize}

All code was developed with the assistance of Claude (Anthropic), an AI coding assistant. The codebase includes 59 unit tests validating the digester model, CHP thermodynamic calculations, energy balance closure, and coupled system integration.
...
At state 8 (turbine exhaust), the steam quality of 0.875 indicates wet steam at the condenser inlet, which is within acceptable limits for steam turbine operation. The relatively high extraction fraction ($y = 0.522$) reflects the substantial thermal demand of heating incoming sludge from \SI{15}{\celsius} to \SI{35}{\celsius}.
...
Figure~\ref{fig:energy} presents the energy flow through the system at baseline conditions. Of the \SI{497}{\kilo\watt} of biogas thermal energy, \SI{423}{\kilo\watt} is transferred to the steam cycle (85\% boiler efficiency), producing \SI{104}{\kilo\watt} of electrical output and \SI{166}{\kilo\watt} of useful process heat. The condenser rejects \SI{148}{\kilo\watt}, and minor losses account for the remainder.
...
Figure~\ref{fig:grid} compares the CHP-integrated plant to grid-only operation on an annual basis. The CHP system reduces grid electricity purchases from \SI{2190}{\mega\watt\hour\per\year} to \SI{1278}{\mega\watt\hour\per\year}, saving approximately \$91,000 per year and avoiding 365 tonnes of CO\textsubscript{2} emissions annually.
...
The self-sufficiency optimization finds that a reactor volume of \SI{7157}{\cubic\metre} (with proportional feed rate at 20-day HRT) produces exactly enough electrical power to meet the \SI{250}{\kilo\watt} WWTP demand. The maximum-power and maximum-efficiency optimizations converge to the same operating point---higher boiler pressure (\SI{12}{\mega\pascal}), higher superheat (\SI{550}{\celsius}), and lower extraction pressure (\SI{200}{\kilo\pascal})---yielding a 20\% improvement in electrical output over baseline.

\subsection{Breakeven Analysis}

Figure~\ref{fig:breakeven} shows the required reactor volume for electrical self-sufficiency as a function of population served. The relationship is approximately linear: each additional 10,000 people requires roughly \SI{700}{\cubic\metre} of additional reactor volume. A plant serving 50,000 people requires approximately \SI{3500}{\cubic\metre}, while one serving 250,000 people requires approximately \SI{18000}{\cubic\metre}.
...
\begin{itemize}[noitemsep]
  ...
  \item \textit{Improved insulation} to reduce wall losses (though these are already modest at \SI{18}{\kilo\watt})
\end{itemize}

Implementing a sludge-digestate heat exchanger could recover approximately 50\% of the sludge heating load, reducing $y$ from 0.52 to roughly 0.30 and increasing electrical output by an estimated 25--30\%.
...
The biogas energy yield of \SI{0.7}{} to \SI{2.0}{\kilo\watt\hour\per\cubic\metre} of biogas reported by Cheng and Wang (2013), as cited in \citet{nathia2018}, corresponds to our computed value of approximately \SI{6.0}{\kilo\watt\hour\per\cubic\metre} of methane (or \SI{3.6}{\kilo\watt\hour\per\cubic\metre} of biogas at 60\% CH\textsubscript{4}), which falls within the upper range. Our CHP electrical efficiency of 20.9\% is consistent with typical small-scale steam Rankine systems, which generally achieve 15--30\% electrical efficiency depending on scale and operating conditions.
...
This project was completed with the assistance of Claude (Anthropic, Claude Opus 4.6 model), used as an AI coding and analysis assistant through the Claude Code CLI tool.

\paragraph{How AI was used:}
\begin{itemize}[noitemsep]
  \item \textbf{Code generation}: Claude generated the Python simulation code (digester model, CHP Rankine cycle model, driver script) based on detailed specifications and thermodynamic equations discussed collaboratively.
  \item \textbf{Unit testing}: Claude wrote 59 unit tests covering the digester model, CHP thermodynamic calculations, energy balance closure, and system integration.
  \item \textbf{Report writing}: Claude drafted this LaTeX report based on the simulation results, with the student providing direction on structure, emphasis, and technical content.
  \item \textbf{Literature integration}: Claude incorporated data and equations from the provided research papers into the models and report.
\end{itemize}

\paragraph{What the student contributed:}
\begin{itemize}[noitemsep]
  \item Project selection and scope definition
  \item Selection of baseline parameters (methane production rate, VFA concentration) from literature
  \item System architecture decisions (steam Rankine CHP, CSTR digester type)
  \item Review and validation of simulation results
  \item Direction on optimization objectives and analysis priorities
\end{itemize}
